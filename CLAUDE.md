...
```bash
scripts/verdict --agent --acknowledge-evidence-egress <path-to-evidence.evtx>
```

- Backend defaults to the direct **Anthropic Messages API** (`--agent-provider anthropic`) with `claude-sonnet-4-6`. It requires evidence-egress acknowledgement and resolves either `ANTHROPIC_API_KEY` or a usable Claude Code OAuth credential without requiring the Claude CLI. `--agent-provider claude_cli` is explicit opt-in, requires the CLI plus supported authentication, and defaults to `claude-opus-4-8`. The OpenAI-compatible providers (`openai`, `openrouter`, `local`, `dgx`) have no universal default model, so pass `--agent-model <id>`; they do not require Claude credentials or the Claude CLI during preflight, while their factories enforce provider-specific keys/endpoints. `local` defaults to `http://localhost:11434/v1`, while `dgx` requires `FINDEVIL_AGENT_BASE_URL`. Cloud providers require egress acknowledgement; `local`/`dgx` do not.
- Strict Phase 4 native scope is one EVTX file. Every non-EVTX type and directory fails closed before launcher preflight or direct-engine MCP execution; use deterministic mode for all other evidence.
...
- Status: live-verified on one EVTX file at report-QA parity with the deterministic engine. Every non-EVTX type and directory fails closed before launcher preflight or direct-engine MCP execution. The `claude_cli` backend does **not** yet scale to disk-sized investigations (per-turn cost); use the deterministic engine for disk/memory.
