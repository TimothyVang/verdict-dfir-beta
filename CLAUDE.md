...
- Status: live-verified on single-file evidence (e.g. an EVTX) at report-QA parity with the deterministic engine. Directory evidence fails closed before MCP startup. The `claude_cli` backend does **not** yet scale to disk-sized investigations (per-turn cost); use the deterministic engine or an efficient OpenAI-compatible endpoint for disk/memory.
