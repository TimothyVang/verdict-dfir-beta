...
3. **`manifest_verify.json.overall == true` and `signature_verified == true`** for the
   authenticated Ed25519/Sigstore tier (audit chain + Merkle root + leaf count + signature all OK).
...
| 4 | Velociraptor `.zip` | `scripts/verdict evidence/<coll>.zip` | zip extracted safely (zip-slip / oversize rejected); per-artifact tools ran; Findings cite `tool_call_id`; manifest verifies | **WORKS** to the extent the 57 typed product tools reach the carried artifacts; classes they don't cover are a documented gap, not `NO_EVIL` |
