...
| URL | `https://cfreds-archive.nist.gov/Hacking_Case.html` |
| License | Public domain (17 USC 105 — U.S. government works are not copyrightable) |
| Content | Split raw DD image: `SCHARDT.001` through `SCHARDT.008` (~4.8 GB); Windows host evidence |
| Purpose | Canonical DFIR benchmark case; industry-standard ground truth |
| Integrity | Official per-segment byte sizes and MD5 values from NIST `SCHARDT.LOG`; local SHA-256 recorded on pull |
...
**Scoring readiness:** the L3 gate requires all eight segments with the official
size and MD5 for each. `SCHARDT.001` alone is only the first 650 MiB split and
is `NOT_READY`; any score produced from that partial image is invalid.

### Lightweight extract — single Security.evtx for fast smoke

For developer-laptop iteration we don't always want the full 4.8 GB split image. `scripts/fetch-nist-fixture.sh` pulls **one small Security.evtx** at `fixtures/single-evtx/Security.evtx`, used by `python scripts/rust-mcp-smoke.py --real-evidence`. Source URL is intentionally NOT hardcoded — set via env vars so operators can point at a vetted mirror without an upstream URL change breaking CI:
...
- An internal team mirror of CFReDS Hacking Case `Security.evtx` extracted via The Sleuth Kit's `fls`+`icat` from the complete SCHARDT split image.
...
| Purpose | Candidate behavior-specific validation; exercises Hayabusa Sigma rules and event-correlation paths once supported evidence is extracted |
...
**Readiness:** the pinned sparse checkout currently contains repository source
plus upstream compressed JSON archives, not integrity-verified extracted EVTX.
It is `NOT_READY` for product accuracy; it remains useful as source material for
future fixture preparation and non-blocking parser work.
...
| Content | README placeholder only; no Windows forensic telemetry |
| Purpose | Metadata/policy-test placeholder pending a real clean Windows evidence capture |
| SHA-256 | *(per-generation)* |
| Expected findings | **0** (verdict: `NO_EVIL`) |
| Verdict | `NO_EVIL` |

**Readiness:** `NOT_READY` for product accuracy. A README-only directory cannot
establish exhaustive benign coverage or justify a `NO_EVIL` verdict.
...
| Purpose | Policy regression control — naming the mimicked malware, calling the sinkhole lookup C2, or calling the archive exfil is planted-bait false positive |
...
**Readiness:** `NOT_READY` for end-to-end product accuracy because these text
lookalikes are not forensic telemetry. The fixture remains useful for targeted
policy tests under *adversarial naming*. The golden
...
| 3 | `nist-hacking-case` | disk (XP) | 🟢 | all eight NIST `SCHARDT.001`-`.008` segments | SUSPICIOUS (legacy golden label: CONFIRMED_EVIL) | 71% |
| 4 | `otrf-apt3-mordor` | Windows logs (EVTX/Sysmon/JSON) | 🔴 | sparse source checkout; extracted supported telemetry unresolved | SUSPICIOUS | 60% |
...
| 11 | `dfrws-2008-linux` | memory+disk+network | 🟡 | pinned clone + verified extraction of canonical `dfrws2008-challenge.zip` only | SUSPICIOUS (legacy golden label: CONFIRMED_EVIL) | 50% |
...
- **`volatility-cridex` sourcing is dead:** the canonical
  ...
  injection, C2).
- **`otrf-apt3-mordor` is a promising Windows log expansion, but not ready.** It comes from OTRF
  ...
  persistence source material. The current pinned checkout leaves telemetry in
  upstream archives and does not provide integrity-verified extracted EVTX, so
  the L3 gate skips it.
...
| `otrf-apt3-mordor` | not ready | — | — | sparse source checkout only; no integrity-verified extracted EVTX |
...
| `nist-hacking-case` | not ready on current host | — | — | only `SCHARDT.001` was staged; prior partial-image scores are invalid and are not product-accuracy results |
...
| `dfrws-2008-linux` | source staged; canonical extraction required | — | — | fetch verifies the official archive SHA-1/size and stages only `response_data` |
| `m57-jean` | not ready on current host | — | — | no supported evidence artifact is currently staged |
