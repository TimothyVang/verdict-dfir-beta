...
2. **Custom MCP Server** — two purpose-built MCP servers expose the typed tool surface:
   - `findevil-mcp` (Rust) — 33 DFIR primitives (core Windows memory/disk/log/network verbs plus allow-listed long-tail wrappers such as `vol_run`, `ez_parse`, `plaso_parse`, `mac_triage`, and `cloud_audit`). Read-only on evidence; SHA-256 every output. **NO `execute_shell`.**
...
**Maturity note.** The 33 Rust verbs are implemented as a typed, allow-listed surface. The
...
| Tool count | 46 (33 Rust DFIR + 13 Python crypto/ACH/memory/ACP/expert) | 200+ (dynamic, shell coverage) |
...
The narrow surface is intentional: it reduces the attack surface from "full shell access" to 33 named Rust DFIR operations and 13 Python cryptographic/ACH/memory/ACP/expert operations, enabling an architectural argument that the agent loop never touches shell primitives directly — all actions flow through typed JSON-RPC schema validation.
...
flowchart TB
    ...
    subgraph Trust2["**TRUST BOUNDARY 2** — Two MCP Servers (typed tool surface)"]
        RustMcp["**findevil-mcp** (Rust, hand-rolled MCP 2024-11-05)<br/>33 typed DFIR tools<br/>NO execute_shell<br/>---<br/>core Windows memory/disk/log/network verbs<br/>+ allow-listed long-tail wrappers"]
