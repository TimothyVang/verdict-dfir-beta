...
1. **Direct Agent Extension** — Claude Code IS the interactive agent. The operator runs `scripts/verdict <evidence>` for the zero-LLM one-shot path, or `scripts/find-evil --acknowledge-evidence-egress` at the repo root for consent-gated interactive exploration; `.mcp.json` auto-spawns both MCP servers; Claude Code drives the investigation as supervisor + Pool A/B subagents (native Task mechanism — not `CLAUDE_CODE_FORK_SUBAGENT`, which is a build-time internal and is not used in this product).
2. **Custom MCP Server** — two purpose-built MCP servers expose the typed tool surface:
   - `findevil-mcp` (Rust) — 43 DFIR primitives (core Windows memory/disk/log/network/browser verbs plus allow-listed long-tail wrappers such as `vol_run`, `ez_parse`, `plaso_parse`, `mac_triage`, and `cloud_audit`). Read-only on evidence; SHA-256 every output. **NO `execute_shell`.**
...
**Maturity note.** The 43 Rust verbs are implemented as a typed, allow-listed surface. The
...
Zeek/PCAP and `vol_*` paths. `browser_history` is fixture-tested across its
version-2 artifact variants but does not yet have a committed real-evidence receipt.
...
| Tool count | 57 (43 Rust DFIR + 14 Python crypto/ACH/memory/ACP/expert) | 200+ (dynamic, shell coverage) |
...
The narrow surface is intentional: it reduces the attack surface from "full shell access" to 43 named Rust DFIR operations and 14 Python cryptographic/ACH/memory/ACP/expert operations, enabling an architectural argument that the agent loop never touches shell primitives directly — all actions flow through typed JSON-RPC schema validation.
...
flowchart TB
    ...
    subgraph Trust2["**TRUST BOUNDARY 2** — Two MCP Servers (typed tool surface)"]
        RustMcp["**findevil-mcp** (Rust, hand-rolled MCP 2024-11-05)<br/>43 typed DFIR tools<br/>NO execute_shell<br/>---<br/>core Windows memory/disk/log/network verbs<br/>+ allow-listed long-tail wrappers"]
...
| 4 | Crypto Custody | **Architectural:** manifest signing and Merkle root computation happen inside `findevil-agent-mcp` before any finding is user-visible. Ed25519 is the default and verifies offline only against an externally trusted public-key fingerprint; Sigstore/Rekor is the identity + transparency-log tier and requires exact trusted identity + issuer policy; the pre-A5 OpenTimestamps/Bitcoin tier was removed so `manifest_finalize` is the terminal custody step | Cryptographic |
...
1. Operator runs `scripts/verdict <evidence>` for a one-shot live investigation, or `scripts/find-evil --acknowledge-evidence-egress` at the repo root for interactive mode. The one-shot launcher performs preflight, starts the optional dashboard unless `--no-dashboard` is set, and delegates to the zero-LLM `find-evil-auto` engine. The interactive path uses Claude Code, which reads `.mcp.json`, spawns both MCP servers, and ingests `CLAUDE.md` + `agent-config/*` as system context. Evidence-facing Rust calls fail closed unless the launcher marks the consumer as the local deterministic controller or the operator explicitly acknowledges parsed-evidence model egress.
...
12. Offline replay: `manifest_verify` reproduces the proof end-to-end under externally supplied signer policy (Ed25519 fingerprint, or Sigstore identity + issuer), citing FRE 902(14) with the post-A5 trusted-time limitation: Rekor inclusion is authenticated, but ordinary `integratedTime` is not itself a cryptographically trusted timestamp.
...
|---|---|---|
| MCP server | Python, 8 servers via sift-gateway, 100+ tools | **Two audit-chained product MCP servers** — Rust `findevil-mcp` (43 DFIR tools, including the deliberately-redundant `vol_pslist` + `vol_psscan` pair plus `vol_psxview` for DKOM cross-validation, disk mount/extract helpers, network/log triage, and allow-listed long-tail wrappers) + Python `findevil-agent-mcp` (14 crypto/ACH/memory/ACP/expert-feedback tools); `.mcp.json` has 6 registered servers total, but the 4 non-product helpers emit no Findings; no execute_shell |
| Agent runtime | Custom Python harness | **Claude Code** itself ("Direct Agent Extension" pattern) — no custom orchestrator to maintain |
| Chain-of-custody | Password-gated HMAC (PBKDF2 2M iter) | Ed25519/Sigstore signer tier + Merkle + audit hash chain (technical digital-identification support for FRE 902(14); qualified-person certification + notice still required, with the A5 trusted-time limitation documented) |
