...
mod tests {
    use super::{
        ...
        ArtifactCandidate, ArtifactKind, MmlsPartition,
    };
    ...
    fn selection_over_a_real_image_listing_keeps_the_web_tier() {
        ...
        for line in text.lines() {
            let Some((inode, path)) = parse_fls_line(line) else {
                continue;
            };
            if let Some(class) = classify_artifact_path(&path) {
                candidates.push(cand(class, &inode, &path));
            }
        }
