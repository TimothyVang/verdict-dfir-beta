...
toolchain; Claude Code auto-spawns the servers from `.mcp.json` on session start. The product surface is 46 tools total: 33 Rust DFIR tools in
...
|---|---|---|---|
| Rust + Cargo | 1.88 (pinned in `rust-toolchain.toml`) | builds `findevil-mcp` (33 DFIR tools) | **yes** |
