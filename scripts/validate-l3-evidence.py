...
# Errors that mean "the product missed its bar", as opposed to "this record is unusable".
# Both fail the run; only these say anything about product quality. Keeping them separable is
# what lets a reader tell a real regression from an abandoned evidence file.
PRODUCT_GATE_ERRORS = frozenset(
    {
        "recall.pass must be true",
        "recall.recall_percent must be >= recall.min_recall_percent",
        "run.finding_count must be >= recall.recalled_n",
    }
)
...
def validate_evidence(
    ...
    # Deliberately NOT compared to the commit under test. This record is generated by hand on
    # one commit and validated on every later one, so demanding equality failed permanently and
    # said nothing about the product -- 14 consecutive nightly failures on an unchanged SHA.
    # What actually matters is whether the record is recent enough to still describe the product.
    if max_age_days is not None:
        generated = str(data.get("generated_at_utc") or "")
        try:
            ...
            if age > max_age_days:
                errors.append(
                    f"evidence is stale: generated {age} days ago, limit is {max_age_days}"
                )
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument(
        "--expected-commit",
        help="accepted for compatibility and ignored; see --max-age-days",
    )
    parser.add_argument(
        "--max-age-days",
        type=int,
        help="fail if the record was generated more than this many days ago",
    )
    ...
    errors = validate_evidence(data, max_age_days=args.max_age_days)
    if errors:
        # Separate "the product missed its bar" from "this record is unusable". Both fail, but
        # only the first is a statement about the product, and a reader must not have to guess
        # which kind of red this is.
        product = [e for e in errors if e in PRODUCT_GATE_ERRORS]
        record = [e for e in errors if e not in PRODUCT_GATE_ERRORS]
        if product:
            print(
                "[l3-evidence] PRODUCT GATE NOT MET -- this is a real miss, not a stale file:",
                file=sys.stderr,
            )
            for error in product:
                print(f"[l3-evidence]   ERROR: {error}", file=sys.stderr)
        if record:
            print("[l3-evidence] evidence record problems:", file=sys.stderr)
            for error in record:
                print(f"[l3-evidence]   ERROR: {error}", file=sys.stderr)
        return 1
